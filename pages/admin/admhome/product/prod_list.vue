    ...
    <div class="prod_list">
        <div class="header">
            <div class="title" @click="GetAllItem">
                商品管理
                <el-button plain size="mini" class="ml-2" @click.native="additem">新增</el-button>
            </div>
            <div class="search">
        ...
        <div class="content">
           ...
           <el-table-column
                ...
                <template slot-scope="scope">
                    <img :src="fakeimg" min-width="60" height="60" />
                </template>
            </el-table-column>
            <el-table-column
                prop="name"
...
export default {
    ...
    methods:{
        handleEdit(x,y){
            // console.log(x,y);
            // this.$router.push({path:'/admin/admhome/product/prod_edit',params:{id:123}})
            this.$router.push({name:'admin-admhome-product-prod_edit',params:{index:x,item:y}})
        },
        handleDelete(x,y){
            console.log(x,y);
            this.$store.dispatch('product/removedata',x);
        },
        search(){
